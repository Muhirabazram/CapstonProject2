  ...
  const doUpdateStatus = async () => {
    ...
    try {
      if (newStatus === 'selesai') {
        if (!fileSurat) {
          setDetailError('Admin harus mengupload file surat resmi ketika menyelesaikan pengajuan.')
          setSaving(false)
          return
        }
          ...
          <div className="space-y-5">
            ...
            {newStatus === 'selesai' && (
              <div className="rounded-xl p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
                <p className="text-sm font-medium text-amber-700 dark:text-amber-300 mb-1">Upload Surat Resmi</p>
                <p className="text-xs text-amber-600 dark:text-amber-400 mb-3">Format: .docx, .doc, atau .pdf. Maks. 20MB.</p>
                <input
                  type="file"
                  accept=".docx,.doc,.pdf"
                  onChange={(e) => { setFileSurat(e.target.files[0]); setDetailError('') }}
                  className="block w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-amber-500 file:text-white hover:file:bg-amber-600 file:cursor-pointer"
                  required
                />
                {fileSurat && (
                  <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-2">
                    File dipilih: {fileSurat.name} ({(fileSurat.size / 1024 / 1024).toFixed(2)} MB)
                  </p>
                )}
              </div>
            )}
